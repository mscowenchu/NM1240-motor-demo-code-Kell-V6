...
//20180813
//#ifdef  P_1R_FOC    //Variables for PWM shift used in 1R FOC system
unsigned char u8_Flag_DoPWMShift, u8_Flag_LeftShift, u8_Flag_RightShift;
uint32_t ui32_duty0, ui32_duty2, ui32_duty4;
//#endif


/*-- 1024 2-byte SIN Table. Angle range: 0~360 degree, Q15 Value range:-32767~32767 --*/
...
//1. Do Inv_Park Transfer: Transfer Vd/q to Valfa/beta
//2. Do Modified Inv_Clark Transfer: Transfer Valfa/Vbeta to Vrefx/y/z
//3. Do SVPWM calculation to produce Duty0/2/4
//4. Update the duty to EPWM->CMPDAT[0/2/4]
// pwm_full_scale = C_PWM_FULL_SCALE, pwm_max_duty = C_MAX_PWM_DUTY
//=======================================================================
void Vdq_to_SVPWM_2R(AMotor* Motor, EPWM_T* epwm, int32_t pwm_full_scale, int32_t pwm_max_duty, int32_t sin, int32_t cos)
{
    ...
    uint32_t PWM0H_duty, PWM2H_duty, PWM4H_duty ;    //20180813: Change to uint32_t
...
// pwm_full_scale = C_PWM_FULL_SCALE, pwm_max_duty = C_MAX_PWM_DUTY
//=======================================================================
void Vdq_to_SVPWM_1R(AMotor* Motor, EPWM_T* epwm, int32_t pwm_full_scale, int32_t pwm_max_duty, int32_t sin, int32_t cos)
{
    int32_t Vd, Vq;
    uint32_t PWM0H_duty, PWM2H_duty, PWM4H_duty ;    //20180813: Change to uint32_t
    uint16_t Taoff_Up, Taoff_Down, Tboff_Up, Tboff_Down, Tcoff_Up, Tcoff_Down, zone;
  
    Vd = Motor->cmd.i16_Vd;
    Vq = Motor->cmd.i16_Vq;

    SVPWM_1R(Vd, Vq, sin, cos, pwm_full_scale, C_PWM_SHIFT_CMP0, C_PWM_SHIFT_CMP1, C_MAX_PWM_DUTY, C_PWM_MARGINE,
             Motor, &zone, &Taoff_Up, &Tboff_Up, &Tcoff_Up, &Taoff_Down, &Tboff_Down, &Tcoff_Down);

    //--- Get PWM0/2/4 Duty -----
    PWM0H_duty = ((uint32_t) (pwm_full_scale - Taoff_Up)) + ((uint32_t) (pwm_full_scale - Taoff_Down)<<16);
    PWM2H_duty = ((uint32_t) (pwm_full_scale - Tboff_Up)) + ((uint32_t) (pwm_full_scale - Tboff_Down)<<16);
    PWM4H_duty = ((uint32_t) (pwm_full_scale - Tcoff_Up)) + ((uint32_t) (pwm_full_scale - Tcoff_Down)<<16);
